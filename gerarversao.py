...
docker rmi {filename}:{current_version} || docker rmi {filename}:v{current_version}
...
def create_temp_file(content):
    ...
    # Oculta o arquivo e agenda a exclusão
    os.system(f"attrib +h {temp_file.name}")  # Oculta
...
if __name__ == "__main__":
    filename = "ecommerce"
    port = 8080
    current_version = "1.0.0"  # Atualize conforme a última versão
